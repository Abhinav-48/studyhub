...
app.post('/api/notes', upload.single('file'), (req, res) => {
  ...
  if (blockedUsers.has(author.toLowerCase())) return res.status(403).json({ error: 'You have been blocked.' });
  const note = { id: uuidv4(), author, title, subject: subject || 'General', description: description || '', fileName: req.file.originalname, fileType: req.file.mimetype, fileUrl: `/uploads/${req.file.filename}`, fileSize: req.file.size, uploadedAt: new Date().toISOString(), downloads: 0 };
  notes.unshift(note);
...
app.delete('/api/notes/:id', (req, res) => {
  ...
  if (requester?.toLowerCase() !== ADMIN_NAME) return res.status(403).json({ error: 'Only admin can delete.' });
...
app.post('/api/block', (req, res) => {
  ...
  if (requester?.toLowerCase() !== ADMIN_NAME) return res.status(403).json({ error: 'Only admin can block.' });
...
app.post('/api/unblock', (req, res) => {
  const { requester, targetUser } = req.body;
  if (requester?.toLowerCase() !== ADMIN_NAME) return res.status(403).json({ error: 'Only admin can unblock.' });
